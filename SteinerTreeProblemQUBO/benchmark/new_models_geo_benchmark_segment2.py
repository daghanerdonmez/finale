"""
New models geometric benchmark - Segment 2: benchmark the hybrid and alex QUBO
formulations on geometric (knn) instances produced by Segment 1.

For each (n, k_terminals, k_neighbors, seed) instance both models are run with
...
Results are saved per instance and aggregated per (knn, n, k) combo, with
separate sub-dicts for "hybrid" and "alex".
"""
import glob
import json
import os
...
def _load_gurobi_results():
    ...
    files = sorted(
        glob.glob(os.path.join(results_dir, "new_models_geo_gurobi_*.json"))
    )
    if not files:
        raise FileNotFoundError(
            f"No new_models_geo_gurobi_*.json found in {results_dir}. "
            "Run new_models_geo_benchmark_segment1.py first."
        )
    latest = files[-1]
...
def _aggregate(runs):
    """Aggregate per-instance run dicts into a summary dict for one (knn,n,k) cell."""
    n = len(runs)
    if n == 0:
        return {}
...
def main():
    ...
    json_path = os.path.join(log_dir, f"new_models_geo_qubo_{timestamp}.json")
    txt_path = os.path.join(log_dir, f"new_models_geo_qubo_{timestamp}.txt")
        ...
        for knn in K_NEIGHBORS_LIST:
            for n in NODE_COUNT_LIST:
                for k in TERMINAL_COUNT_LIST:
                    ...
                    results["summary"][f"knn={knn}|n={n}|k={k}"] = combo_summary
